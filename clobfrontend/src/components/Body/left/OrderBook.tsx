    ...
    <div className="bg-zinc-900 h-[507px] rounded-lg p-4 w-full overflow-y-auto flex flex-col justify-start text-sm">
      ...
      {rows.map((row, i) => {
        ...
        <div key={i} className="grid grid-cols-4 py-1 text-white relative">
            {Number(row.buyAmount) > 0 && <div className="absolute h-full left-0 top-0 bg-green-900 opacity-20" style={{width:`${buyPercent}%`}}/>}
            {Number(row.buyAmount) > 0 && <div className="absolute h-full right-0 top-0 bg-red-900 opacity-20" style={{width:`${sellPercent}%`}}/>}
